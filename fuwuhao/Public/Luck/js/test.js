var Calendar = function(isYear,isMonth,isDay){
    ...
    var oCalenWrap = create('div', {"class" : 'calendar'}),           // 最大父级
        oCalenMask = create('div', {"class" : 'calendar-mask_copy'}),           // 灰快遮罩
    ...
    Calendar.prototype.show = function () {
        ...
        // 显示日历
        var _this = aCalendars[0];
        attr(_this, 'readonly', 'true');

        if(attr(_this, 'disabled') != null)return;

        var start = Number(attr(_this, 'start')) || 1915,
            end = Number(attr(_this, 'end')) || 2020;

        past = !(attr(_this, 'past') == null);
        $this.hours = !(attr(_this, 'hours') == null);
        $this.hoursPast = !(attr(_this, 'hours-past') == null);

        $this.shield = getDate(attr(_this, 'shield') || '');
        $this.startDate = getDate(attr(_this, 'start-date') || '');
        ...
        //初始化年
        if(isYear > 0){
            yNow += isYear - oDate.getFullYear();
            $this.selectDate(null, selectYearBox, "y", yNow,1);
        }

        //初始化月
        if(isMonth > 0){
            mNow += (isMonth - 1) - oDate.getMonth() ;
            $this.selectDate(null, selectMonthBox, "m", mNow,1);
        }

        var day_list = $('.calendar-list div').eq(1).find('a');
        day_list.each(function(){
            if(!$(this).hasClass('pasted') && !$(this).hasClass('next-m')){
                if(isDay == $(this).text()){
                    $('.calendar-list a').removeClass('today');
                    $(this).addClass('today');
                }
            }
        })
    ...
    Calendar.prototype.init = function(){
        ...
        $(".calendar_warp").append(oCalenWrap);
        ...
        this.createHeader(function(){
            ...
            for(var i = 0 ; i < calenTitles.length ; i++){
                ...
                calenTitles[i].onclick = function(){
                    //$('.calendar-list').height(210);
                    if(toolClass(this, 'calendar-month-txt', 'has')){
                      //  $('.calendar-months').height(230);
                        // 显示或者隐藏
    ...
    /**/
    Calendar.prototype.selectDate = function(obj, obj2, attr, val ,type){
        var $this = this;
    ...
    //var tester = $.tester = new Calendar();
}

function showDay(day) {
    if(day > 0){
        $('span .weekend').removeClass('today');
        var div = $('.calendar-list').eq(0);
