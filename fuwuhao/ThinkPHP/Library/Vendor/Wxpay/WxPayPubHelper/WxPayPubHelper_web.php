...
*/
	...
	include_once("WxPay.pub.web.config.php");
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	{
	    $xml = $this->createXml();
		$this->response = $this->postXmlCurl($xml,$this->url,$this->curl_timeout);//var_dump($xml);var_dump($this->response);die();
...
{	
	...
	function getResult(){
		$this->postXml();
		return  $this->result = $this->xmlToArray($this->response);
	}
}
...
class OrderQuery_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
	 ...
	 function order_query($out_trade_no){
                ...
                return $this->xmlToArray($this->postXml());
        }
...
class JsApi_pub extends Common_util_pub
{
	...
	function getOpenid()
	{
		...
		$_SESSION['weixin']=null;
	    $_SESSION['weixin']=$data;
	...
	//获取昵称
	public  function getnikia($openid){
	    //获取token
		...
		$par="access_token=".$token['access_token']."&openid=".$openid;
		$url ="https://api.weixin.qq.com/cgi-bin/user/info?".$par;
		//初始化curl
		$ch = curl_init();
		//设置超时
		curl_setopt($ch, CURLOP_TIMEOUT, $this->curl_timeout);
...
class template extends Wxpay_client_pub{
	/**
	 * POST 请求
	 * @param string $url
	 * @param array $param
	...
	//发送模板消息
	public function post_template($openid,$date){
		$template_id = 'pQJyWUFQ5g3z58yQWI8Jc8yqA4QSzsWMabXbTlgW-jI';
		$url = 'http://wx.fuxiyi.com/fuwuhao/index.php/Home/Order/buy.html';
		$data = ['touser'=>$openid,
				 'template_id'=>$template_id,
				 'url'=>$url,
				'data'=>['first'=>['value'=>'尊贵的黄历用户：','color'=>'#173177'],
						  'name'=>['value'=>'我的黄历VIP','color'=>'#173177'],
						  'expDate'=>['value'=>$date,'color'=>'#173177'],
						  'remark'=>['value'=>'为了方便您的正常使用请及时续费。','color'=>'#173177']]]; 
	  $token = $this->get_access_token();
	  $post_url = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$this->token;
	  $res =  $this->http_post($post_url,json_encode($data));
