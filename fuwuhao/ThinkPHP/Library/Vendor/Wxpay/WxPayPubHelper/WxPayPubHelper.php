...
{	
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new SDKRuntimeException("partner_trade_no"."<br>");
			}elseif ($this->parameters["amount"] == null ) {
				throw new SDKRuntimeException("amount"."<br>");
			}elseif ($this->parameters["desc"] == null) {
				throw new SDKRuntimeException("desc"."<br>");
			}elseif ($this->parameters["openid"] == NULL){
				throw new SDKRuntimeException("openid！trade_type为JSAPI时，openid为必填参数！"."<br>");
			}
		   	$this->parameters["mch_appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mchid"] = WxPayConf_pub::MCHID;//商户号
		   	$this->parameters["check_name"] = 'NO_CHECK';//不校检姓名
		   	$this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip	    
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
